...
There are multiple algorithms for rate-limiting <<google-cloud>><<wikipedia-rate-limiting>><<figma-design>>, such as the token bucket, leaky bucket, sliding window, and fixed window algorithms.
...
- [[[figma-design, 8]]] N. Mahdi. "An alternative approach to rate limiting." Medium. https://medium.com/figma-design/an-alternative-approach-to-rate-limiting-f8a06cf7c94c (accessed Aug. 8, 2020).

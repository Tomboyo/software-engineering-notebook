= Exploration of general rate-limiter usage

This example consists of a Spring Boot application utilizing https://github.com/resilience4j/resilience4j[resilience4j] annotations. The application includes a link:src/main/java/com/github/tomboyo/example/PingController.java[REST API] which responds to `GET /` requests after 200 milliseconds and a link:src/main/java/com/github/tomboyo/example/Requester.java[client] which issues such requests on a configurable schedule. The REST API protects itself from excessive use with a resilience4j rate-limiter, which may reject incoming requests before they are serviced:
...
The ratelimiter is configured to permit as many as three reqeusts per window, and is configured to refresh permits once every second (that is, it allows three requests per second). Any request which cannot immediately acquire a permit will block for as long as 10 milliseconds waiting for a permit to become available. If a permit does not become available in this time, the call fails with a 429 code. This configuration resides in the application yaml:
